...
# ════════════════════════════════════════════════════════════════
# GAP FIXES — wire after LiveDispatchFactory and DailyScheduleFactory
# ════════════════════════════════════════════════════════════════
#
# from ai_gap_fixes import AIGapFixesFactory
# from models import OptimizationMode
#
# def on_eta_update(trip_id: str, new_eta: int):
#     # Push updated ETA to frontend via Django Channels / WebSocket
#     channel_layer.group_send(f"trip_{trip_id}", {"eta_minutes": new_eta})
#
# gaps = AIGapFixesFactory.create(
#     monitoring_engine  = live["monitoring_engine"],
#     smart_router       = live["smart_router"],
#     on_eta_update      = on_eta_update,
#     optimization_mode  = OptimizationMode.BALANCED,   # or RELAXED / EFFICIENT
#     eta_interval_sec   = 30
# )
#
# ── Balanced/Relaxed schedule generation (replaces basic optimizer) ──
# entries = gaps["schedule_optimizer"].generate(trips, drivers)
#
# ── Start ETA polling (runs in background thread) ─────────────
# gaps["eta_polling_handler"].start()
# gaps["eta_polling_handler"].register_trip(trip.id, driver, (dest_lat, dest_lng))
# # On trip complete:
# gaps["eta_polling_handler"].unregister_trip(trip.id)
# # On shutdown:
# gaps["eta_polling_handler"].stop()
#
# ── Manual override (dispatcher drags trip on board) ──────────
# override_event = gaps["override_handler"].apply_override(
#     trip               = trip,
#     new_driver         = driver,
#     previous_driver_id = old_driver_id,
#     dispatcher_id      = request.user.id,
#     feedback           = "Driver was closer"
# )
# # Store override_event dict in PostgreSQL via backend dev
# override_log = gaps["override_handler"].get_override_log()
#
# ── Analytics with AI accuracy ────────────────────────────────
# metrics = gaps["analytics_engine"].summary(
#     call_logs       = call_logs,
...
# # ai_accuracy_score | ai_avg_confidence | ai_overrides | top_dismissed_triggers
# return JsonResponse(metrics)


# ════════════════════════════════════════════════════════════════
# ANALYTICS — Provider Portal (basic, without gap fixes)
# ════════════════════════════════════════════════════════════════
#
# from analytics import AnalyticsEngine
#
